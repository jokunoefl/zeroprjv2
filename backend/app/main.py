...
def get_dependencies(subject: str, db: Session = Depends(get_db)):
    """指定された科目の依存関係データを取得"""
    try:
        if subject.lower() == "math":
            try:
    ...
    try:
        if subject.lower() == "math":
            ...
            return {
                "subject": "math",
                "levels": level_groups,
                "topics": list(dependency_map.values())
            }
...
@app.get("/check-math-dependencies")
def check_math_dependencies(db: Session = Depends(get_db)):
    """math_dependenciesテーブルの内容を確認するエンドポイント"""
    from sqlalchemy import text
    try:
        # テーブルの行数を確認
